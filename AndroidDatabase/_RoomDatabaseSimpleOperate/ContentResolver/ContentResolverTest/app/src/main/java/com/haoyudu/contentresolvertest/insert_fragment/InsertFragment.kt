package com.haoyudu.roomcrud.insert_fragment
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        btn_confirm.setOnClickListener{
            if (edit_name!=null&&edit_age!=null){
                val student= Student(edit_name.text.toString(),"小明",edit_age.text.toString().toInt())
                StudentDao(requireContext()).insert(student)
                Navigation.findNavController(it).navigate(R.id.mainFragment)
            }else{
                Toast.makeText(it.context,"信息未完善",Toast.LENGTH_SHORT).show()
            }
        }
    }
